...
import be.raft.pelican.application.managers.ServerBuildManager;
import be.raft.pelican.requests.Route;
import be.raft.pelican.requests.action.AbstractManagerBase;
...
public class ServerBuildManagerImpl extends AbstractManagerBase implements ServerBuildManager {
	...
	public ServerBuildManagerImpl(ApplicationServer server, ApplicationImpl impl) {
	...
	protected RequestBody finalizeData() {
		...
		if (shouldUpdate(OOM)) obj.put("oom_disabled", !oom);

		JSONObject limits = new JSONObject()
				.put(
						"memory",
						shouldUpdate(MEMORY) ? memory : server.getLimits().getMemory())
				...
				.put(
						"threads",
						shouldUpdate(THREADS) ? threads : server.getLimits().getThreads());
		obj.put("limits", limits);
		JSONObject featureLimits = new JSONObject()
				.put(
						"databases",
						shouldUpdate(DATABASES)
								? databases
								: server.getFeatureLimits().getDatabases())
				.put(
						"allocations",
						shouldUpdate(ALLOCATIONS)
								? allocations
								: server.getFeatureLimits().getAllocations())
				.put(
						"backups",
						shouldUpdate(BACKUPS)
								? backups
								: server.getFeatureLimits().getBackups());
